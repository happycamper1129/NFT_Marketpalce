...
const CollectionTitleDescription = React.memo<PropTypes>(({title, description}) => {
    return (
        <div className="flex flex-col text-center items-center my-2 space-y-3 px-2 lg:max-w-[70%]">
            <div className="font-archivo font-black text-5xl">{title}</div>
            <div className="font-archivo font-bold text-md opacity-70">{description}</div>
        </div>
    );
});
